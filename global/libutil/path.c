...
/**
 * regularrize: regularrize path name
 */
const char *
regularrize(const char *path, const char *cwd)
{
	static char regular_path_name[MAXPATHLEN];

	if (isabspath(path)) {
		char *q = locatestring(path, cwd, MATCH_AT_FIRST);
		...
		if (path[0] == '.' && path[1] == '/')
			snprintf(regular_path_name, MAXPATHLEN, "%s", path);
		else
			snprintf(regular_path_name, MAXPATHLEN, "./%s", path);
	}
	return regular_path_name;
}
/**
